...
- 所有回應文字必須使用{language}
         ...
         - priorities：必須只從職缺內容中挑出重要關鍵技能，不能包含職缺中未提及的技能！每個職缺會不一樣！每個技能要包含explanation說明為何得分是這樣。如果職缺要求特定年數經驗，必須根據實際年數給分（例如：要求8年但只有5年，最多給60-70%）。如果履歷中明確提到相關經驗，應該給出合理的高分（70-90%），不要過於保守
